...
class intNN_oscillator(integrator):

    def __init__(self, tstart, tend, nsteps, problem):
        super(intNN_oscillator, self).__init__(tstart, tend, nsteps, problem)
        curr_file = os.path.dirname(__file__)
        netfile = curr_file + f"/../networks/osc/Net_a_{problem.a}_{self.dt:.4f}"
        try:
            f = open(netfile, "rb")
            ...
            model.train(problem.generateData(1000, self.dt), 100000, model_path=netfile)
            self.net = model
